...
function SpellWord({ route }) {
  ...
  //fix this, trying is find if the are words not completed in current list
  ...
  function nextWord() {
    ...
    if (!oneInThree()) {
      ...
      const randomWrongWord = getRandomWrongWord();
      setwordObject((prev) => return {
        wordObject: randomWrongWord, prev.index;
      });
      return;
    }
