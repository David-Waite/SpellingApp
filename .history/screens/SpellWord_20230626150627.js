...
function SpellWord({ route }) {
  ...
  const DUMMYWORD = useSelector(
    (state) => state.words.wordLists[0].words[1].Word
  );
  ...
  async function submitSpellingForm(formData) {
    ...
    console.log(state.words.wordLists[0].words[1].Completed);
  }
    ...
    >
      <View style={styles.container}>
        ...
        <SpellingForm
          ...
          value={enteredWord}
        />
        <SpellButtons definiton={DUMMYWORDDEF} word={DUMMYWORD} />
      </View>
      {wordCompleted !== null && (
        <WordPopUpBtn
