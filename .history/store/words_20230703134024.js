...
export const wordsSlice = createSlice({
  ...
  reducers: {
    update: (state, action) => {
      console.log(
        state.learningList.words.findIndex((wordInList) => {
      ...
      const wordStateIndex = wordListState.words.findIndex((wordInList) => {
        return wordInList.word === action.payload.wordObject.word;
      });
      const wordObjectState = wordListState.words[wordStateIndex];
      console.log(wordObjectState);

      if (completedAction) {
        if (learningWordIndexState !== -1) {
          const learningWordObjectState =
            state.learningList.words[learningWordIndexState];
          learningWordObjectState.ttd - 1;
          console.log(learningWordObjectState.ttd);
          if (learningWordObjectState.ttd === 0) {
            const index = learningList.words.findIndex(
              (wordInList) => wordInList.word === learningWordObjectState
            );
